...
const { asteroidHull, pointInHull, edgeDistSq, bulletHitHull, rand, wrap } = require("./geometry")
...
function makeAsteroid(g, r, x, y, vx, vy) {
  g.asteroids.push({ id: g.aidSeq++, x, y, vx, vy, rotX: 0, rotY: 0, r, size: ASTEROID_R.indexOf(r) })
}
...
function splitAsteroid(g, a) {
  ...
  if (idx < 2) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      makeAsteroid(g, r, a.x, a.y, Math.cos(angle) * speed, Math.sin(angle) * speed)
    }
  }
}
...
function updateGame(g, dt, inputs) {
  ...
  if (!p.active) {
    ...
    p.y += p.vy * dt
    const m = 3
    if (p.x > BOUNDS + m || p.x < -BOUNDS - m || p.y > BOUNDS + m || p.y < -BOUNDS - m) {
      p.active = false
      p.timer = rand(20, 45)
    }
  }
    ...
    for (let i = g.asteroids.length - 1; i >= 0; i--) {
      ...
      if (!shipHit) {
        for (let k = 0; k < hull.length; k++) {
          ...
          if (edgeDistSq(s.x, s.y, hull[k][0], hull[k][1], hull[l][0], hull[l][1]) < SHIP_R * SHIP_R) {
            shipHit = true
            break
          }
        }
  ...
  if (alive.length === 0 && Object.keys(g.ships).length > 0) {
    console.log(`game over at step ${g.step}, ${Object.keys(g.ships).length} ship(s)`)
    g.running = false
  }
}
...
function serializeState(g) {
  ...
  const planet = gplanet.active
    ...
    : { active: false }
  return { t: g.step, ships, asteroids: ast, bullets: bul, particles: par, planet, running: g.running, level: g.level }
}

module.exports = { createGame, updateGame, serializeState, initPlayerShip, initLevel }
