...
function renderRoomList(rooms) {
  ...
  for (const r of rooms) {
    ...
    const label = r.inGame
      ? `In game (${r.players}/${r.max})`
      : `${r.players}/${r.max}`;
...
function getOrCreateShip(pid, color) {
  ...
  d.group.material.color.setHex(color);
  return d;
}
...
const PARTICLE_COLORS = [
  0xff3333, 0xff6633, 0xffaa33, 0xffff33, 0x33ff33, 0x33ffff, 0x3388ff,
  0xff33ff, 0xff66aa, 0xffffff,
];
...
function updateAttract(dt) {
  ...
  if (attractPlanet.y > BOUNDS + m || attractPlanet.y < -BOUNDS - m)
    attractPlanet.vy *= -1;
}
...
function updateLocalShip(dt) {
  ...
  const spd = Math.sqrt(
    localShip.vx * localShip.vx + localShip.vy * localShip.vy,
  );
  if (spd > 6) {
...
function render(dt) {
  ...
  if (gameActive && st) {
    ...
    if (me && me.invuln > 0)
      myShip.group.visible = Math.floor(me.blink) % 2 === 0;
